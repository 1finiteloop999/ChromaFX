...
namespace ChromaFX
{
    ...
    public static class ColorTheoryEngine
    {
        ...
        public static bool RunSelfTest()
        {
            ...
            // 1. MAIN严格等于种子
            Check("MAIN equals seed color", (Color)s.Get(ColorRole.Main) == new Color(seed.r, seed.g, seed.b, 1f));
            // 2. 明度秩序（在L*空间校验，与生成空间一致）
            Check("L* order HL > MA > SH", L(ColorRole.Highlight) > L(ColorRole.Main)
                                    && L(ColorRole.Main) > L(ColorRole.Shadow));
            // 3. 三族曲线L*单调
            Check("all three curves monotonic", pal.Validate(out _));
            // 4. Primary曲线基准点 ≈ 种子色（曲线基准即用户输入）
            Check("Primary at TBase equals seed", ColorSpaces.DeltaE00(
            ...
            Check("Accent base comes from template",
                ColorSpaces.HueDistance(pal.accentBaseHueHsv, expectedAccentHue) < 0.01f);
            // 6. 互补模式下ACCENT与MAIN色相距离显著
            Check("accent contrast", ColorSpaces.HueDistance(acHsv.x, mainHsv.x) > 90f);
            ...
            Check("neutral chroma within cap", neutralC <= EffectColorProfile.Default.neutralChromaCap + 0.5f);
            // 8. AMBIENT饱和度低于MAIN
            Check("ambient less saturated than main", amHsv.y < mainHsv.y);
            ...
            Check("analogous accent stays in sector", accDist <= 46.81f);
            ...
            Check("higher tone gives lower L*", La > Lb && Lb > Lc);
            // 11. 对比度缩放L*跨度
            ...
            Check("contrast scales L* span", SpanOf(pHi) > SpanOf(pLo));
            ...
            foreach (var scheme in GenerateRecommendedSchemes(seed))
            {
                ...
                string curve = "[ChromaFX]   Primary curve t=0->1: ";
                for (int i = 0; i <= 5; i++)
            ...
            Debug.Log(allPass
                ? "[ChromaFX] ColorTheoryEngine self-test passed."
                : "[ChromaFX] ColorTheoryEngine self-test FAILED.");
            return allPass;
        }
    }
}
